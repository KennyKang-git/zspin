...
Artifact       : zs\_m62\_verify\_v1\_4\_1.py   (one-command run: python3 zs\_m62\_verify\_v1\_4\_1.py)

Result         : 90 rows PASS / 0 FAIL, exit 0        \[FULL profile\]
...
Controls         : G \= 18 (fail-closed guards, incl. manuscript integrity, scope consistency,

                           the statement guard for Theorems 2 and 17(iv), the SELF-IDENTIFYING

                           digest row M8, and the SELF-REFERENTIAL guard-invariance row M9)
...
Profiles         : FULL  50-75 s single core \-- the only profile that may be quoted
                   ...
                   QUICK \~25 s single core   \-- python3 zs\_m62\_verify\_v1\_4\_1.py \--quick

                   IDENTIFY  \< 1 s          \-- python3 zs\_m62\_verify\_v1\_4\_1.py \--identify
...
Self-reference   : TWO rows have this manuscript's own verification apparatus as their subject.
                   ...
                   previous delivery, by the guard suite.
...
paper\_code/version  : ZS-M62 v1.4.1
...
verification artifact : zs\_m62\_verify\_v1\_4\_1.py  (90 rows, 0 FAIL)
...
| `F-M62.29` | two different contents published under one version label, or a version increment used to hide a mathematical change | **active**; every release carries its own whole-document digest, and a patch release must additionally exhibit the *unchanged* part digests of the release it patches. `v1.4.1` does so for parts `B` and `C` |
...
main\_script          : zs\_m62\_verify\_v1\_4\_1.py

identity-only mode   : python3 zs\_m62\_verify\_v1\_4\_1.py \--identify \[manuscript.md\]
...
ledger               : zs\_m62\_verify\_v1\_4\_1.json

manuscript           : DISCOVERED, not hard-coded.  The script takes an optional .md argument;

                       otherwise it globs its own directory for ZS-M62\_v1\_4\_1.md, then ZS\*M62\*.md.
...
profiles             : FULL   python3 zs\_m62\_verify\_v1\_4\_1.py          50-75 s, ledger

                              certificate=true.  This is the only profile that may be quoted.

                       QUICK  python3 zs\_m62\_verify\_v1\_4\_1.py \--quick  \~25 s, reduced sample

                              counts, ledger certificate=false, written to a SEPARATE file

                              zs\_m62\_verify\_v1\_4\_1\_quick.json so it can never overwrite the
...
one-command run      : python3 zs\_m62\_verify\_v1\_4\_1.py

expected row count   : 90 rows, 0 FAIL, exit 0   (identical in both profiles)

sha256(script)       : d09f0e86aad77092b95de69cdc277650cc8794384c5ec5a782367aa096748228

sha256(ledger, FULL) : fbd3994ab169a8f775f3ce4875e9f91b84296bd2788b131df3339aa117bffc6b

transport digest     : 525ae6de95258175cece0b04d1f5e52f50f2143ae87dfafd55ede537e0cc1d7d

part digest A        : ce705ea686a43391e368a2c541f2e12a770a8bf233b45ce5cf01921eff615708
...
part digest D        : 327a82a452e0fc7e958dffc4b6c6586e297e22ea5cd8577be2d28dba9431c540

part digest E        : 3b667e3150e5e99c03852e99afe92bb3190f1829aae2d9654e994ee5b446bb9a
...
fail-closed          : row-count guard, class-census guard, AST self-audit (no literal-True in
                       ...
                       any failure returns exit 1

known limitations    : grid linear programmes are discretizations, tolerances declared per row;
...
Guards that never fire are not guards. Thirty-four attacks have been executed across v1.1–v1.4.1; every one behaved as designed, and the four that did *not* behave as designed on first execution are recorded below with their failed outcome, because that is where the errata came from. Attacks `A9a`–`A13` are the audit-driven additions; `A1`, `A6`–`A8` were carried over from v1.0 and re-executed against v1.1.
...
Attacks `A1`–`A13` were executed in the full profile; `A14`–`A17` in the quick profile, which exercises the same guards with reduced sample counts. Attacks `A23`–`A29` are the v1.4 additions and were executed in the quick profile, except `A29`, which was executed in both. Attacks `A30`–`A34` are the v1.4.1 additions and were executed in the quick profile; `A32` runs no profile at all.
...
verification route  : every displayed identity is either given a written proof in this manuscript or
                      ...
                      recomputed independently in zs\_m62\_verify\_v1\_4\_1.py; the first-order closed form
