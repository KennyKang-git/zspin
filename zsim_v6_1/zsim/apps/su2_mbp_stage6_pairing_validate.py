...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Run stage-6 pairing-aware MBP validation for Z-Sim v3.7.')
    p.add_argument('--shape', default='2,2,2')
    p.add_argument('--output-dir', default='outputs/su2_mbp_stage6_pairing_validate')
    ...
    p.add_argument('--scan-schemes', default='reduced2,staggered2')
...
def run_su2_mbp_stage6_pairing_validate(
    *,
    shape: tuple[int, int, int] = (2, 2, 2),
    output_dir: str | Path = 'outputs/su2_mbp_stage6_pairing_validate',
    ...
    best, best_links, scan_rows = scan_stage6_valley_family(
        lattice,
    ...
    controls = stage6_pairing_controls(
        lattice,
    ...
    identity_pair = float(controls['identity']['harness']['pair_score_mean'])
    scrambled_pair = float(controls['scrambled_caloron']['harness']['pair_score_mean'])
    gates = {
        'G-PAIRING-LOCALIZATION': best_objective > median_objective,
        'G-HARNESS-FINITE': all((row.gamma_consistency_gap == row.gamma_consistency_gap) and (row.pair_score == row.pair_score) for row in scheme_rows),
        'G-FD-CONSISTENCY': float(harness['gamma_gap_max']) < 2.5e-1,
        'G-PAIRING-VS-IDENTITY': float(harness['pair_score_mean']) > identity_pair,
        'G-SCRAMBLED-DISTINCT': abs(float(harness['pair_score_mean']) - scrambled_pair) > 1.0e-8,
        'G-SCAN-COVERAGE': len(scan_rows) == len(amplitudes) * len(separations) * len(widths) * len(seam_biases),
    }
    out_dir = ensure_output_dir(output_dir)
    write_csv_rows(out_dir / 'stage6_scan_rows.csv', [row.to_dict() for row in scan_rows])
    write_csv_rows(out_dir / 'stage6_scheme_rows.csv', [row.to_dict() for row in scheme_rows])
    write_json(out_dir / 'stage6_pairing_summary.json', {
        'shape': list(shape),
        'projector_mode': projector_mode,
        'chirality_mode': chirality_mode,
        ...
        'gates': gates,
        'notes': {
            'stage': 'pairing-aware valley ranking + scheme comparison harness',
            'status': 'preproduction-surrogate',
            'non_claim': 'not a continuum caloron, not a production-scale 8^3 lattice, not final Higgs bilinear closure',
        },
    })
    return {
        'success': True,
        ...
        'generated_outputs': ['stage6_scan_rows.csv', 'stage6_scheme_rows.csv', 'stage6_pairing_summary.json'],
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return print_cli_failure('Z-Sim v3.7 stage-6 pairing validation failed.', exc)
    return print_cli_summary('Z-Sim v3.7 stage-6 pairing validation complete.', summary, ordered_keys=('success', 'shape', 'scan_schemes', 'compare_schemes', 'num_scan_rows', 'num_scheme_rows', 'output_dir'))
