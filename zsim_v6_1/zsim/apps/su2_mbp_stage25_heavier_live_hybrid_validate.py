...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Run the stage-25 heavier live-hybrid sweep validation harness for Z-Sim v5.6.')
    p.add_argument('--reference-stage24-summary', default='outputs/su2_mbp_stage24_live_hybrid_example/stage24_summary.json')
    ...
    p.add_argument('--output-dir', default='outputs/su2_mbp_stage25_heavier_live_hybrid_example')
    return p


def run_su2_mbp_stage25_heavier_live_hybrid_validate(
    *,
    ...
    output_dir: str | Path = 'outputs/su2_mbp_stage25_heavier_live_hybrid_example',
) -> dict[str, object]:
    summary, payload = run_stage25_pipeline(
        reference_stage24_summary=reference_stage24_summary,
    ...
    write_csv_rows(out_dir / 'stage25_live_rows.csv', [row.to_dict() for row in summary.sweep_rows])
    write_csv_rows(out_dir / 'stage25_control_rows.csv', [row.to_dict() for row in summary.sweep_rows if row.is_control])
    write_json(out_dir / 'stage25_summary.json', payload)
    return {
        'success': True,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = run_su2_mbp_stage25_heavier_live_hybrid_validate(
            reference_stage24_summary=args.reference_stage24_summary,
        ...
        print_cli_failure('su2_mbp_stage25_heavier_live_hybrid_validate', exc)
        return 1
    print_cli_summary('su2_mbp_stage25_heavier_live_hybrid_validate', result)
    return 0
