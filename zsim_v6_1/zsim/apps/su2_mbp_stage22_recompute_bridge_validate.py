...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Run the stage-22 recompute-enabled bridge validation harness for Z-Sim v5.3.')
    p.add_argument('--reference-stage21-summary', default='outputs/su2_mbp_stage21_default_live_example/stage21_summary.json')
    ...
    p.add_argument('--output-dir', default='outputs/su2_mbp_stage22_recompute_bridge_example')
    return p


def run_su2_mbp_stage22_recompute_bridge_validate(
    *,
    ...
    output_dir: str | Path = 'outputs/su2_mbp_stage22_recompute_bridge_example',
) -> dict[str, object]:
    summary, payload = run_stage22_pipeline(
        reference_stage21_summary=reference_stage21_summary,
        reference_stage20_summary=reference_stage20_summary,
        reference_stage13_summary=reference_stage13_summary,
    )
    out_dir = ensure_output_dir(output_dir)
    write_csv_rows(out_dir / 'stage22_recompute_rows.csv', [row.to_dict() for row in summary.sweep_rows])
    write_csv_rows(out_dir / 'stage22_control_rows.csv', [row.to_dict() for row in summary.sweep_rows if row.is_control])
    write_json(out_dir / 'stage22_summary.json', payload)
    return {
        'success': True,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = run_su2_mbp_stage22_recompute_bridge_validate(
            reference_stage21_summary=args.reference_stage21_summary,
        ...
        print_cli_failure('su2_mbp_stage22_recompute_bridge_validate', exc)
        return 1
    print_cli_summary('su2_mbp_stage22_recompute_bridge_validate', result)
    return 0
