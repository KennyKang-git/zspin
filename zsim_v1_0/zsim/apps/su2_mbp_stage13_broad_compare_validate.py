...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Run stage-13 broad-vs-lightweight comparison validation for Z-Sim v1.0.')
    p.add_argument('--broad-shape-grid', default='1,1,1;2,1,1;2,2,1;2,2,2;3,2,1;3,2,2;3,3,1')
    p.add_argument('--lightweight-shape-grid', default='1,1,1;2,1,1;2,2,1')
    p.add_argument('--output-dir', default='outputs/su2_mbp_stage13_broad_compare_validate')
...
def run_su2_mbp_stage13_broad_compare_validate(
    *,
    ...
    output_dir: str | Path = 'outputs/su2_mbp_stage13_broad_compare_validate',
    ...
    write_csv_rows(out_dir / 'stage13_preset_rows.csv', [r.to_dict() for r in summary.preset_rows])
    write_csv_rows(out_dir / 'stage13_comparison_rows.csv', [r.to_dict() for r in summary.comparison_rows])
    write_json(out_dir / 'stage13_summary.json', payload)
    return {
        'success': True,
        'broad_shape_grid': tuple(broad_shape_grid),
        ...
        'num_preset_rows': len(summary.preset_rows),
        'num_comparison_rows': len(summary.comparison_rows),
        'output_dir': str(out_dir),
        'generated_outputs': ['stage13_preset_rows.csv', 'stage13_comparison_rows.csv', 'stage13_summary.json'],
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return print_cli_failure('Z-Sim v1.0 stage-13 broad-vs-lightweight comparison validation failed.', exc)
    return print_cli_summary('Z-Sim v1.0 stage-13 broad-vs-lightweight comparison validation complete.', summary, ordered_keys=('success', 'broad_shape_grid', 'lightweight_shape_grid', 'selected_shape', 'selected_sign_method', 'selected_scheme', 'selected_source_label', 'num_preset_rows', 'num_comparison_rows', 'output_dir'))
