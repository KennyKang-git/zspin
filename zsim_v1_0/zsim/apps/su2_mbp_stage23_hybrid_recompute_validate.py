...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Run the stage-23 hybrid recompute validation harness for Z-Sim v1.0.')
    ...
    p.add_argument('--output-dir', default='outputs/su2_mbp_stage23_hybrid_recompute_example')
    return p


def run_su2_mbp_stage23_hybrid_recompute_validate(
    *,
    ...
    output_dir: str | Path = 'outputs/su2_mbp_stage23_hybrid_recompute_example',
) -> dict[str, object]:
    ...
    write_csv_rows(out_dir / 'stage23_hybrid_rows.csv', [row.to_dict() for row in summary.sweep_rows])
    write_csv_rows(out_dir / 'stage23_control_rows.csv', [row.to_dict() for row in summary.sweep_rows if row.is_control])
    write_json(out_dir / 'stage23_summary.json', payload)
    return {
        'success': True,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = run_su2_mbp_stage23_hybrid_recompute_validate(
        ...
        print_cli_failure('su2_mbp_stage23_hybrid_recompute_validate', exc)
        return 1
    print_cli_summary('su2_mbp_stage23_hybrid_recompute_validate', result)
    return 0
