...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Run stage-12 closure-calibration validation for Z-Sim v1.0.')
    p.add_argument('--shape-grid', default='1,1,1;2,1,1;2,2,1;2,2,2;3,2,1;3,2,2;3,3,1')
    p.add_argument('--output-dir', default='outputs/su2_mbp_stage12_calibration_validate')
...
def run_su2_mbp_stage12_calibration_validate(
    *,
    shape_grid: Sequence[tuple[int, int, int]] = ((1, 1, 1), (2, 1, 1), (2, 2, 1), (2, 2, 2), (3, 2, 1), (3, 2, 2), (3, 3, 1)),
    output_dir: str | Path = 'outputs/su2_mbp_stage12_calibration_validate',
    ...
    write_csv_rows(out_dir / 'stage12_coverage_rows.csv', [r.to_dict() for r in summary.coverage_rows])
    write_csv_rows(out_dir / 'stage12_calibration_rows.csv', [r.to_dict() for r in summary.calibration_rows])
    write_json(out_dir / 'stage12_summary.json', payload)
    return {
        'success': True,
        ...
        'num_coverage_rows': len(summary.coverage_rows),
        'num_calibration_rows': len(summary.calibration_rows),
        'output_dir': str(out_dir),
        'generated_outputs': ['stage12_coverage_rows.csv', 'stage12_calibration_rows.csv', 'stage12_summary.json'],
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return print_cli_failure('Z-Sim v1.0 stage-12 closure-calibration validation failed.', exc)
    return print_cli_summary('Z-Sim v1.0 stage-12 closure-calibration validation complete.', summary, ordered_keys=('success', 'shape_grid', 'selected_shape', 'selected_sign_method', 'selected_scheme', 'num_coverage_rows', 'num_calibration_rows', 'output_dir'))
