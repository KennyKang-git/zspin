...
from zsim.lgt.stage9 import run_stage9_pipeline


def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Run stage-9 shape/control validation for Z-Sim v1.0.')
    p.add_argument('--shape-grid', default='1,1,1;2,1,1')
    p.add_argument('--output-dir', default='outputs/su2_mbp_stage9_shape_control_validate')
...
def run_su2_mbp_stage9_shape_control_validate(
    *,
    shape_grid: Sequence[tuple[int, int, int]] = ((1, 1, 1), (2, 1, 1)),
    output_dir: str | Path = 'outputs/su2_mbp_stage9_shape_control_validate',
    ...
    write_csv_rows(out_dir / 'stage9_shape_rows.csv', [r.to_dict() for r in summary.shape_rows])
    write_csv_rows(out_dir / 'stage9_control_rows.csv', [r.to_dict() for r in summary.control_rows])
    write_json(out_dir / 'stage9_summary.json', payload)
    return {
        'success': True,
        ...
        'num_control_rows': len(summary.control_rows),
        'output_dir': str(out_dir),
        'generated_outputs': ['stage9_shape_rows.csv', 'stage9_control_rows.csv', 'stage9_summary.json'],
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        summary = run_su2_mbp_stage9_shape_control_validate(
        ...
        return print_cli_failure('Z-Sim v1.0 stage-9 shape/control validation failed.', exc)
    return print_cli_summary('Z-Sim v1.0 stage-9 shape/control validation complete.', summary, ordered_keys=('success', 'shape_grid', 'selected_shape', 'selected_sign_method', 'selected_scheme', 'num_shape_rows', 'num_control_rows', 'output_dir'))
