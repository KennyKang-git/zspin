"""Z-Sim v1.0 — CMB Power Spectrum Evaluation.
...
def evaluate_zspin(As: float = 2.1e-9, tau: float = 0.054,
    ...
    if verbose:
        print("=" * 65)
        print("  Z-Sim v1.0 — CMB Evaluate (MCMC-free)")
...
def scan_As(tau: float = 0.054, As_min: float = 1.5e-9,
    ...
    if verbose:
        print("=" * 65)
        print("  Z-Sim v1.0 — A_s Grid Scan")
...
def main(argv=None):
    ...
    if args.output:
        try:
            ...
            ax1.set_title('Z-Sim v1.0 — CMB Evaluate (MCMC-free)')
            ax1.legend()
