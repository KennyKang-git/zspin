...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Run stage-10 direct comparison validation for Z-Sim v1.0.')
    p.add_argument('--shape-grid', default='1,1,1;2,1,1;2,2,1')
    p.add_argument('--output-dir', default='outputs/su2_mbp_stage10_compare_validate')
...
def run_su2_mbp_stage10_compare_validate(
    *,
    shape_grid: Sequence[tuple[int, int, int]] = ((1, 1, 1), (2, 1, 1), (2, 2, 1)),
    output_dir: str | Path = 'outputs/su2_mbp_stage10_compare_validate',
    ...
    write_csv_rows(out_dir / 'stage10_shape_rows.csv', [r.to_dict() for r in summary.shape_rows])
    write_csv_rows(out_dir / 'stage10_comparison_rows.csv', [r.to_dict() for r in summary.comparison_rows])
    write_json(out_dir / 'stage10_summary.json', payload)
    return {
        'success': True,
        ...
        'generated_outputs': ['stage10_shape_rows.csv', 'stage10_comparison_rows.csv', 'stage10_summary.json'],
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        summary = run_su2_mbp_stage10_compare_validate(
        ...
        return print_cli_failure('Z-Sim v1.0 stage-10 direct comparison validation failed.', exc)
    return print_cli_summary('Z-Sim v1.0 stage-10 direct comparison validation complete.', summary, ordered_keys=('success', 'shape_grid', 'selected_shape', 'selected_sign_method', 'selected_scheme', 'num_shape_rows', 'num_comparison_rows', 'output_dir'))
